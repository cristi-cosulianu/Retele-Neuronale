...
def problema1(a, b):
    seta = set(a)
    setb = set(b)
    return tuple([seta.intersection(setb), seta.union(setb), seta.difference(setb), setb.difference(seta)])
...
def validate_dict(tuples, dictionary):
    for tup in tuples:
        value = dictionary.get(tup[0])
        if value:
            if not value.startswith(tup[1]):
                return False
            if not value.endswith(tup[-1]):
                return False
            value.replace(tup[1], "")
            value.replace(tup[-1],"")
            if not value.find(tup[2]):
                return False
        else:
            return False
    return True
...
def problem6(operator, a, b):
    function = operators_dict.get(operator)
    if function:
        return function(a, b)
    return None
